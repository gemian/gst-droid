...
gst_droidadec_data_available (void *data, DroidMediaCodecData * encoded)
{
  ...
  GST_DEBUG_OBJECT (dec, "data available of size %lu", encoded->data.size);
...
push:
  GST_DEBUG_OBJECT (dec, "pushing %lu bytes out", gst_buffer_get_size (out));
...
gst_droidadec_handle_frame (GstAudioDecoder * decoder, GstBuffer * buffer)
{
  ...
  GST_DEBUG_OBJECT (dec, "decoding data of size %lu (%ud)",
